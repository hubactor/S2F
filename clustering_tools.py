...
def draw_clusters3d(X, labels, colors=None, show_plt=True, show_title=False, name=None, ax=None, markersize=15,
...
def print_dists_statistics(dists):
    print
    "mean: %.3f" % (np.mean(dists))
    print
    "median: %.3f" % (np.median(dists))
    print
    "variance: %.3f" % (np.var(dists))
    print
    "max: %.3f" % (np.max(dists))
    print
    "min: %.3f" % (np.min(dists))
    print
    "std+mean: %.3f" % (np.std(dists) + np.mean(dists))
...
evaulations_dict = {
    ...
    EvaluationFields.clusters_num: lambda X, labels_true, labels, name, params: len(np.unique(labels)) - (
        1 if -1 in labels else 0),
    EvaluationFields.normazlied_mutual_information: lambda X, labels_true, labels, name,
                                                           params: metrics.normalized_mutual_info_score(labels_true,
                                                                                                        labels),
    EvaluationFields.adjusted_mutual_information: lambda X, labels_true, labels, name,
                                                         params: metrics.adjusted_mutual_info_score(labels_true,
                                                                                                    labels),
    EvaluationFields.adjusted_rand_index: lambda X, labels_true, labels, name, params: metrics.adjusted_rand_score(
        labels_true, labels),
...
class MethodsEvaluation:
    ...
    def print_evaluations(self, draw_in_notebook=False):
        print
        "\t".join(CLUSTERS_EVALUATION_COLUMNS)
